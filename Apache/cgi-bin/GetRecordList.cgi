...
#===============================================================================
#
# [Admin Tool] Contributor毎の化合物名リストを取得
#
...
$sql = "select substring(NAME,1,instr(NAME,';')-1) as compound, "
			...
			."where S.ID=R.ID group by compound order by compound";
@ans1 = &MySql( $sql );
foreach $rec1 ( @ans1 ) {
	my $compound = @$rec1[0];
	$compound =~ s/\'/\\\'/g;
	$sql = "select ID, NAME from SPECTRUM where substring(NAME,1,instr(NAME,';')-1)='$compound' order by ID";
	@ans2 = &MySql( $sql );
	my @ids = ();
	foreach $rec2 ( @ans2 ) {
		push(@ids, "@$rec2[0] NAME=@$rec2[1]");
	}
	print join("\t", @$rec1), "\t", join("@", @ids), "\n";
}
$dbh->disconnect;
exit(0);
