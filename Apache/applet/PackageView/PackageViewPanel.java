...
public class PackageViewPanel extends JPanel {
	...
	private JButton paramBtn = null;						// パラメータ設定ボタン
	
	private boolean isParamWindow = false;				// パラメーター設定ウィンドウフラグ
	
	private static int CUTOFF = 5;						// CutOff入力値
	private static float TOLERANCE = 0.3f;				// Tolerance入力値
	...
	public static final String COL_LABEL_QUERY = "Query";
	...
	public static final String INTEGRATE_RECORD = "Integrate";
	 ...
	 */
	public void addRecInfoAfter(int sortKey) {
		recNum = specData.getRecNum();
		if (recNum == 0) {
			return;
		}
		allBtnCtrl(true);
		specData.setMatchPeakInfo(CUTOFF, TOLERANCE, isTolUnit);
	...
	private void initComponentLayout() {
		...
		pkgPane.setDividerLocation((int)(PackageView.initAppletHight * 0.7));
	...
	private JTable createRecListTable() {
		...
		TableSorter recSorter = new TableSorter(new DefaultTableModel(), specData);
		JTable t = new JTable(recSorter) {
			...
			public void setValueAt(Object value, int row, int col) {
				...
				int matchCol = recTable.getColumnModel().getColumnIndex(COL_LABEL_MATCH);
				int hitCol = recTable.getColumnModel().getColumnIndex(COL_LABEL_HIT);
				
				// ラジオボタン及び、チェックボックスが編集された場合に再描画を行う
				if (recTable.getColumnName(col).equals(COL_LABEL_QUERY)) {
					PackageRecData recData = specData.getRecInfo(row);
					recData.setQueryRecord(Boolean.parseBoolean(String.valueOf(recTable.getValueAt(row, col))));
					specData.setMatchPeakInfo(CUTOFF, TOLERANCE, isTolUnit);
					PackageViewPanel.this.repaint();
					
					for (int i=0; i<recTable.getRowCount(); i++) {
						// 編集されたレコードによってMatch及び、Hitカラムの値をアップデートする
						setValueAt(specData.getRecInfo(i).getMatchPeakNum(), i, matchCol);
						setValueAt(specData.getRecInfo(i).getHitPeakNum(), i, hitCol);
					}
				}
				else if (recTable.getColumnName(col).equals(COL_LABEL_DISABLE)) {
					PackageRecData recData = specData.getRecInfo(row);
			...
			public void paintComponent(Graphics g) {
				...
				if (chgColor.isSelected()) {
					...
					for (int i=0; i<recTable.getRowCount(); i++) {
						// 非表示の場合は色づけしない
						if (Boolean.parseBoolean(String.valueOf(getValueAt(i, getColumnModel().getColumnIndex(COL_LABEL_DISABLE))))) {
							continue;
						}
						g2.setPaint(getColor(i));
						g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.6f));
						
						area.setRect(getCellRect(i, getColumnModel().getColumnIndex(COL_LABEL_ORDER), false));
						g2.fill(area);
					}
				}
				...
				for (int i=0; i<recTable.getRowCount(); i++) {
					if (isSortStatus(COL_LABEL_QUERY)) {
						area.setRect(getCellRect(i, getColumnModel().getColumnIndex(COL_LABEL_QUERY), false));
						g2.fill(area);
					}
					else if (isSortStatus(COL_LABEL_DISABLE)) {
						area.setRect(getCellRect(i, getColumnModel().getColumnIndex(COL_LABEL_DISABLE), false));
						g2.fill(area);
					}
					else if (isSortStatus(COL_LABEL_HIT) || isSortStatus(COL_LABEL_MATCH)) {
						area.setRect(getCellRect(i, getColumnModel().getColumnIndex(COL_LABEL_QUERY), false));
						g2.fill(area);
						area.setRect(getCellRect(i, getColumnModel().getColumnIndex(COL_LABEL_DISABLE), false));
						g2.fill(area);
		...
		((TableSorter)t.getModel()).setTableHeader(t.getTableHeader());
		...
		// カラムセット
		DefaultTableModel model = (DefaultTableModel)((TableSorter)t.getModel()).getTableModel();
		model.setColumnIdentifiers(columnLabel);

		// 列幅セット
		t.getColumn(t.getColumnName(0)).setPreferredWidth(36);	// Order
		t.getColumn(t.getColumnName(1)).setPreferredWidth(38);	// Query
		t.getColumn(t.getColumnName(2)).setPreferredWidth(360);	// Name
		t.getColumn(t.getColumnName(3)).setPreferredWidth(28);	// Hit
		t.getColumn(t.getColumnName(4)).setPreferredWidth(38);	// Match
		t.getColumn(t.getColumnName(5)).setPreferredWidth(48);	// Disable
		t.getColumn(t.getColumnName(6)).setPreferredWidth(36);	// Peak
		t.getColumn(t.getColumnName(7)).setPreferredWidth(60);	// Precursor
		t.getColumn(t.getColumnName(8)).setPreferredWidth(72);	// ID
	...
	private void setTblData() {
		...
		for (int i=0; i<recTable.getColumnCount(); i++) {
			((TableSorter)recTable.getModel()).setSortingStatus(i, TableSorter.NOT_SORTED);
		}
		
		DefaultTableModel dataModel = (DefaultTableModel)((TableSorter)recTable.getModel()).getTableModel();
		...
		for (int i=0; i<recNum; i++) {
			...
			recordData[1] = new Boolean(recData.isQueryRecord());
			recordData[2] = recData.getName();
			recordData[3] = recData.getHitPeakNum();
			recordData[4] = recData.getMatchPeakNum();
			recordData[5] = new Boolean(recData.isDisable());
			recordData[6] = String.valueOf(recData.getPeakNum());
			recordData[7] = recData.getPrecursor();
			recordData[8] = recData.getId();
	...
	private void setTblDispStatus(boolean disable) {
    	if (recTable.getRowCount() > 0) {
    		int disableCol = recTable.getColumnModel().getColumnIndex(COL_LABEL_DISABLE);
			for (int i=0; i<recTable.getRowCount(); i++) {
	...
	private boolean isSortStatus() {
		return ((TableSorter)recTable.getModel()).isSorting();
	}
	
	/**
	 * レコードリストテーブルソート状態取得
	 * レコードリストテーブルの指定されたカラムがソートされているかを取得する。
	 * @param columnName カラム名
	 * @return ソート状態(true：ソート中、fale：デフォルト)
	 */
	private boolean isSortStatus(String columnName) {
		// TableSorterに渡すカラムインデックスは初期カラム位置のインデックス
		int column = -1;
		for (int i=0; i<columnLabel.length; i++) {
			if (columnLabel[i].equals(columnName)) {
				column = i;
			}
		}
		
		int sortStatus = ((TableSorter)recTable.getModel()).getSortingStatus(column);
	...
	/**
	 * レコードリスト用ポップアップ表示非表示使用可否取得
	 * @param action 動作(true：表示、false：非表示)
	 * @return 使用可否(true：使用可、false：使用不可)
	 */
	private boolean getPopupDispStatus(boolean action) {
		// 特定カラムのソート状態時と0件の場合は使用不可とする
		if (isSortStatus(COL_LABEL_DISABLE)
				|| isSortStatus(COL_LABEL_HIT)
				|| isSortStatus(COL_LABEL_MATCH)
				|| recTable.getRowCount() == 0) {
			return false;
		}
		
		// 1件でもactionと異なる状態がある場合に使用可とする
		int disableCol = recTable.getColumnModel().getColumnIndex(COL_LABEL_DISABLE);
		for (int i=0; i<recTable.getRowCount(); i++) {
			if(action != Boolean.parseBoolean(String.valueOf(recTable.getValueAt(i, disableCol)))) {
				return true;
			}
		}
    	return false;
	...
	private void recListPopup(MouseEvent e) {
		int rowCnt = recTable.getRowCount();

		JMenuItem item1 = new JMenuItem("All Disable");
		item1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				setTblDispStatus(true);
			}
		});
		item1.setEnabled(getPopupDispStatus(true));
		
		JMenuItem item2 = new JMenuItem("All Enable");
		item2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				setTblDispStatus(false);
			}
		});
		item2.setEnabled(getPopupDispStatus(false));
		
		JSeparator sp = new JSeparator();
		
		JMenuItem item3 = new JMenuItem("Sorting Cancel");
					...
					if (animationTimer == null || !animationTimer.isRunning()) {
						...
						if (inPeakArea) {
							for (int k=0; k<overCursorMz.size(); k++) {
								if (Double.parseDouble(String.valueOf(overCursorMz.get(k))) == mz) {
									isOnLine = true;
									break;
								}
							}
...
//				float xs = (getWidth() - 2.0f * MARGIN) / massRange;
		...
		class ContextPopupListener implements ActionListener {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (com.equals("search")) {
					...
					// JSP呼び出し
					String reqUrl = PackageView.baseUrl + "jsp/Result.jsp" + urlParam.toString();
					try {
						PackageView.context.showDocument(new URL(reqUrl), "_blank");
					}
					catch (Exception ex) {
						ex.printStackTrace();
					}
	...
	class ButtonPane extends JPanel implements MouseListener, MouseMotionListener {
		...
		private ParameterWindow paramWin = null;
		...
		public ButtonPane() {
			...
			paramBtn = new JButton("compare parameter");
			paramBtn.setName("param");
			paramBtn.addMouseListener(this);
			paramBtn.setMargin(new Insets(0, 0, 0, 0));
		...
		public void mouseClicked(MouseEvent e) {
			...
			if ( SwingUtilities.isLeftMouseButton(e) ) {
				...
				else if (btnName.equals("param")) {
					// 子画面が開いていなければ生成
					if (!isParamWindow) {
						paramWin = new ParameterWindow();
					} else {
						paramWin.requestFocus();
					}
				}
			}
		}
	...
	class TblMouseListener extends MouseAdapter {
		...
		public void mouseClicked(MouseEvent e) {
			...
			// Hit、Matchカラムソート中は選択処理を行わない
			if (isSortStatus(COL_LABEL_HIT) || isSortStatus(COL_LABEL_MATCH)) {
				return;
			}
			
			// ラジオボタン及び、チェックボックスの選択処理
			int queryCol = recTable.getColumnModel().getColumnIndex(COL_LABEL_QUERY);
			int disableCol = recTable.getColumnModel().getColumnIndex(COL_LABEL_DISABLE);
			if (selCol == queryCol) {
				if (isSortStatus(COL_LABEL_QUERY)) {
					return;
				...
				for (int i=0; i<recTable.getRowCount(); i++) {
					if (i == selRow) {
						recTable.setValueAt(
								!Boolean.parseBoolean(String.valueOf(recTable.getValueAt(selRow, queryCol))),
								selRow, 
								queryCol);
						if (Boolean.parseBoolean(String.valueOf(recTable.getValueAt(selRow, queryCol)))
								&& recTable.getRowCount() > 1) {
					...
					recTable.setValueAt(new Boolean(false), i, queryCol);
				}
				recTable.setRowSelectionInterval(selRow, selRow);
			}
			else if (selCol == disableCol) {
				if (isSortStatus(COL_LABEL_DISABLE)) {
					return;
				}
				recTable.setValueAt(
	...
	class TblRenderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value,
			...
			int queryCol = table.getColumnModel().getColumnIndex(COL_LABEL_QUERY);
			int disableCol = table.getColumnModel().getColumnIndex(COL_LABEL_DISABLE);
			...
			// 非表示レコードの文字色変更
			if (Boolean.parseBoolean(String.valueOf(table.getValueAt(row, disableCol)))) {
				if (isSelected) {
	 ...
	 * ParameterWindowクラス
	 */
	class ParameterWindow extends JFrame {
		
		private JRadioButton tolUnit1 = new JRadioButton("unit", isTolUnit);
		private JRadioButton tolUnit2 = new JRadioButton("ppm", isTolPpm);
		...
		public ParameterWindow() {
			...
			final JTextField toleranceField = new JTextField(String.valueOf(TOLERANCE), 3);
			keyListenerList.add(toleranceField);
			...
			tolerancePanel.add(toleranceField);
			tolerancePanel.add(tolUnit1);
			tolerancePanel.add(tolUnit2);

			add(panel1, title, constraints, 0, 0, 1, 1);
			add(panel1, tolerancePanel, constraints, 1, 0, 1, 1);
			add(panel1, ex, constraints, 2, 0, 1, 1);
			...
			add(panel1, title, constraints, 0, 1, 1, 1);
			add(panel1, cutoffField, constraints, 1, 1, 1, 1);
			add(panel1, ex, constraints, 2, 1, 1, 1);
			...
			okButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					final float tolerance = TOLERANCE;
					final int cutoff = CUTOFF;
					try {
						TOLERANCE = Float.parseFloat(toleranceField.getText());
					} catch (Exception ex) {
					...
					if (tolerance != TOLERANCE
							|| isTolUnit != tolUnit1.isSelected()
							|| isTolPpm != tolUnit2.isSelected()
							|| cutoff != CUTOFF) {
					...
					for (int i=0; i<recTable.getRowCount(); i++) {
						// 編集されたレコードによってMatch及び、Hitカラムの値をアップデートする
						recTable.setValueAt(specData.getRecInfo(i).getMatchPeakNum(), i, matchCol);
						recTable.setValueAt(specData.getRecInfo(i).getHitPeakNum(), i, hitCol);
					}
				}
			});
			cancelButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					// 選択状態を元に戻す
					if (isTolUnit != tolUnit1.isSelected()) {
						tolUnit1.setSelected(isTolUnit);
					}
					if (isTolPpm != tolUnit2.isSelected()) {
						tolUnit2.setSelected(isTolPpm);
					}
					dispose();
					isParamWindow = false;
				}
